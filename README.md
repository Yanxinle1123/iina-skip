...
- 视频指纹分析通过比对帧哈希（dHash）寻找各集共享的片头/片尾画面。
- 可选「自动跳过」，无需鼠标即可跳过。
...
匹配器：
- 使用 ffmpeg 每秒提取 2 帧，缩放到 9x8 灰度图并计算感知哈希（dHash）。
- 逐帧比对相邻集对应位置的哈希值（汉明距离），寻找连续低距离的共享画面段。
...
- 视频指纹匹配同样分析前 6 分钟和后 6 分钟的画面，并接受时长介于 20 到 150 秒之间的共享段。
